...
export const createAppointment = async (req, res) => {
  try {
    ...
    // Enviar el correo
    await sendConfirmationEmail(email, tokenConfirmacion);

    res.status(201).json({ message: "Cita creada y correo de confirmación enviado", box: boxAsignado });
...
export const confirmarCita = async (req, res) => {
  try {
    ...
    return res.redirect(`https://amaris-front-production.up.railway.app/confirmar-cita/${token}`);
